...
func TestLivePresent(t *testing.T) {
	...
	provider, err := NewDNSProviderCredentials(ctx, ovhEndpoint, ovhApplicationKey, ovhApplicationSecret, ovhConsumerKey, util.RecursiveNameservers)
	require.NoError(t, err)

	err = provider.Present(ctx, ovhDomain, "_acme-challenge."+ovhDomain+".", "123d==")
	require.NoError(t, err)
}
...
func TestLiveCleanUp(t *testing.T) {
	...
	err = provider.CleanUp(ctx, ovhDomain, "_acme-challenge."+ovhDomain+".", "123d==")
	require.NoError(t, err)
}
